...
gulp.task('copy_nuget_dlls', gulp.series('nuget_restore', function() {
    var copyAzureBlobStorage = gulp.src(['./packages/Microsoft.Azure.Storage.Blob.11.2.3/lib/net452/Microsoft.Azure.Storage.Blob.dll']).pipe(gulp.dest('./lib/ps_modules/NugetPackages'));
    var copyAzureFileStorage = gulp.src(['./packages/Microsoft.Azure.Storage.File.11.2.3/lib/net452/Microsoft.Azure.Storage.File.dll']).pipe(gulp.dest('./lib/ps_modules/NugetPackages'));
    ...
    var copyIdentityClient = gulp.src(['./packages/Microsoft.Identity.Client.4.74.0/lib/net462/Microsoft.Identity.Client.dll']).pipe(gulp.dest('./lib/ps_modules/NugetPackages'));
    ...
    return merge(copyAzureBlobStorage, copyAzureFileStorage, copyDataMovement, copyApplicationsInsight, copyTracingEventSource, copyThreadingTasks, copyNewtonsoft, copyVstsClient, copyIdentityClient, copyIdentityAbstraction, copyTfsWebApi, copyTfsCommonContract, copyAspNetWebApi);
}));
