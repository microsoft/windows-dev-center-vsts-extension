function Start-Publishing
{
    ...
    if ($IsSparseBundle) 
    {
        ...
        for($i = 0; $i -lt $allApps.Length; $i++)
        {
            ...
            {
                $dlcJsonObject = @{
                    'ProductId' = (Get-Product -AppId $appIdMinimatchPackages[0] -Verbose:$useVerbose -AccessToken $AccessToken -NoStatus).id
...
function Publish-DLCApp
{
    ...
    $null = Wait-ProductPackageProcessed -ProductId $productId -SubmissionId $SubmissionId -AccessToken $AccessToken -NoStatus:$NoStatus -Verbose:$useVerbose
...
function Watch-ExistingSubmission 
{
    ...
    {
        Start-SubmissionMonitor -Product $productId -SubmissionId $submissionID -AccessToken $AccessToken -Verbose:$useVerbose -NoStatus
